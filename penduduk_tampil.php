 ...
 <section class="content">
      <div class="row">
        <div class="col-md-12">
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title"><a href="penduduk_add.php">Tambah</a>  |  Master Data Penduduk</h3>
            </div>
        ...
        <?php
            ...
            $query = $_POST['query'];
            if ($query !=''){
                $select = mysqli_query($konek, "SELECT * FROM v_penduduk WHERE nik LIKE '%".$query."%' OR nama LIKE '%".$query."%' OR jenis LIKE '%".$query."%' ");
            }else {
               $select=mysqli_query($konek,"SELECT * FROM `v_penduduk` ORDER BY `jenis` ASC");
            }
